...
static void RtmpConnection_connectCallback(void *ptr, ttLibC_TettyPromise *promise) {
...
bool ttLibC_RtmpConnection_connect(
	...
	conn_->bootstrap = ttLibC_TettyBootstrap_make();
	ttLibC_TettyBootstrap_channel(conn_->bootstrap, ChannelType_Tcp);
	ttLibC_TettyBootstrap_option(conn_->bootstrap, Option_SO_KEEPALIVE);
	ttLibC_TettyBootstrap_option(conn_->bootstrap, Option_TCP_NODELAY);

	// setup pipeline.
	conn_->handshake = ttLibC_RtmpHandshake_make();
	ttLibC_TettyBootstrap_pipeline_addLast(conn_->bootstrap, (ttLibC_TettyChannelHandler *)conn_->handshake);
	conn_->decoder = ttLibC_RtmpDecoder_make();
	ttLibC_TettyBootstrap_pipeline_addLast(conn_->bootstrap, (ttLibC_TettyChannelHandler *)conn_->decoder);
	conn_->encoder = ttLibC_RtmpEncoder_make();
	ttLibC_TettyBootstrap_pipeline_addLast(conn_->bootstrap, (ttLibC_TettyChannelHandler *)conn_->encoder);
	conn_->command_handler = ttLibC_RtmpCommandHandler_make();
	ttLibC_TettyBootstrap_pipeline_addLast(conn_->bootstrap, (ttLibC_TettyChannelHandler *)conn_->command_handler);
	conn_->client_handler = ttLibC_RtmpClientHandler_make();
	ttLibC_TettyBootstrap_pipeline_addLast(conn_->bootstrap, (ttLibC_TettyChannelHandler *)conn_->client_handler);

	// do connect.
	ttLibC_TettyBootstrap_connect(conn_->bootstrap, server, port);

	// wait until handshake done.
	ttLibC_TettyPromise *handshake_promise = ttLibC_RtmpHandshake_getHandshakePromise(conn_->bootstrap, conn_->handshake);

	ttLibC_TettyPromise_await(handshake_promise);
	ttLibC_TettyPromise_close(&handshake_promise);

	// send connect message.
	ttLibC_Amf0Command *connect = ttLibC_Amf0Command_connect(address, app);
	ttLibC_TettyPromise *connect_promise = ttLibC_TettyBootstrap_makePromise(conn_->bootstrap);

	ttLibC_TettyPromise_addEventListener(connect_promise, RtmpConnection_connectCallback, conn_);
	connect->promise = connect_promise;

	ttLibC_TettyBootstrap_channels_write(conn_->bootstrap, connect, sizeof(ttLibC_Amf0Command));
	ttLibC_TettyBootstrap_channels_flush(conn_->bootstrap);
	ttLibC_Amf0Command_close(&connect);

	ttLibC_TettyPromise_await(connect_promise);
	ttLibC_TettyPromise_close(&connect_promise);
	return true;
}

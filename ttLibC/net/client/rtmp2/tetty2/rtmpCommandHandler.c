...
static tetty2_errornum RtmpCommandHandler_channelRead(
	...
	case RtmpMessageType_amf0Command:
		{
			...
			if(strcmp((const char *)command->command_name, "_result") == 0) {
				...
				if(send_command->promise != NULL) {
					if(strcmp((const char *)send_command->command_name, "createStream") == 0) {
						if(command->obj2 != NULL && command->obj2->type == amf0Type_Number) {
							...
							ttLibC_Tetty2Promise_setSuccess(send_command->promise, &passData);
						}
					}
					else {
						ttLibC_Tetty2Promise_setSuccess(send_command->promise, command);
					}
				}
			}
				...
				if(rtmpStream != NULL && rtmpStream->promise != NULL) {
					ttLibC_Tetty2Promise_setSuccess(rtmpStream->promise, command);
				}
			}
			else {
...
static tetty2_errornum RtmpCommandHandler_write(
		ttLibC_Tetty2Context *ctx,
		void *data,
		size_t data_size) {

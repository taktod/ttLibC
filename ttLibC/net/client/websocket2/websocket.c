...
ttLibC_WebSocket TT_VISIBILITY_DEFAULT *ttLibC_WebSocket_make(const char *address) {
	...
	socket->bootstrap = ttLibC_TcpBootstrap_make();

	socket->handshake_handler = ttLibC_WebSocketHandshakeHandler_make();
	ttLibC_Tetty2Bootstrap_pipeline_addLast(socket->bootstrap, socket->handshake_handler);
	socket->handler = ttLibC_WebSocketHandler_make();
	ttLibC_Tetty2Bootstrap_pipeline_addLast(socket->bootstrap, socket->handler);

	ttLibC_TcpBootstrap_connect(socket->bootstrap, socket->server, socket->port);
...
bool TT_VISIBILITY_DEFAULT ttLibC_WebSocket_update(
	...
	ttLibC_TcpBootstrap_update(socket_->bootstrap, wait_interval);
...
0x80 0x02 0x6c 0x6f ( "lo" )
	...
	*b8 =  0x80 | opcode; // flag for message begining and opcode.
	...
	ttLibC_Tetty2Bootstrap_write(socket_->bootstrap, buf, send_size);
	...
	// send mask
	ttLibC_Tetty2Bootstrap_write(socket_->bootstrap, mask, 4);
	...
	for(size_t i = 0;i < data_size;) {
		...
		++ i;
		if(i != 0 && i % 256 == 0) {
			ttLibC_Tetty2Bootstrap_write(socket_->bootstrap, buf, 256);
		}
	}
	ttLibC_Tetty2Bootstrap_write(socket_->bootstrap, buf, data_size % 256);
...
void TT_VISIBILITY_DEFAULT ttLibC_WebSocket_close(ttLibC_WebSocket **socket) {
	...
	ttLibC_Tetty2Promise_close(&target->handshake_promise);

...
ttLibC_TheoraEncoder *ttLibC_TheoraEncoder_make(
	...
	th_comment tc;
	th_info_init(&ti);
	ti.frame_width = width;   // should be x16?
	ti.frame_height = height; // should be x16?
	ti.pic_width = width;
...
bool ttLibC_TheoraEncoder_encode(
	...
	// cb
	encoder_->image_buffer[1].width  = yuv420->inherit_super.width >> 1;
	encoder_->image_buffer[1].height = yuv420->inherit_super.height >> 1;
	encoder_->image_buffer[1].stride = yuv420->u_stride;
	encoder_->image_buffer[1].data   = yuv420->u_data;
	// cr
	encoder_->image_buffer[2].width  = yuv420->inherit_super.width >> 1;
	encoder_->image_buffer[2].height = yuv420->inherit_super.height >> 1;
